...
**Unresolved.** One item only, and it is closed rather than open: M16 ("In Motion") has no authoritative-channel upload to find, so its "Not sourced yet" stub is permanent (searched twice, 12 August 2026). The other 19 works are sourced and verified across 15 listening slots. Lesson 11 was rewritten on 12 August 2026 and is no longer a stub. The repo is public at `Edwards-Resources/year11-music-2027` and live on GitHub Pages. `canvasUrl` stays null until Matthew creates the 2027 Canvas course; the nav renders that honestly as "Canvas (course not yet built)".
