...
- **Strand Yellow** (`#FFD23F`, Term 2, Creative practice): Term 2's strand colour, and also the fixed colour of the assessment-flag row (`.at`, the AT1/AT2/AT3 row in a rail or schedule) and the `.note` aside (teaching asides, adjustments, "where this fits" pointers, content-pending notices) on every page regardless of which term is on air. The build accepts this dual duty deliberately, the css comment on `.note` records that the flag colour is the same yellow everywhere so notes and assessment rows read alike across all three terms, with a 1px ink border added specifically to keep the note distinct from an actual Term 2 strand fill.
...
### Notes / Flags
`.note`: fixed yellow fill (`--s2`) with a 1px ink border, used for teaching asides, differentiation adjustments, "where this fits" pointers and content-pending stubs on every term regardless of which strand colour is on air.

**The note is the loudest device on the page, so it is spent sparingly.** On 16 August 2026 four of Term 1's notes carried an ICIP protocol notice, three of them the same paragraph verbatim. That text was written to the teacher ("work in partnership with Elders, Community members, Cultural Knowledge Holders or your local AECG"), which a student cannot action, and on Lessons 1 and 11 its own trigger condition was not even met: Lesson 1's works are Mozart, Puente, Piazzolla and Buena Vista Social Club, and Lesson 11 is a vocabulary review. The three generic instances were removed. One was kept, on Lesson 5, reworded into student voice and retitled "Using First Nations material in your work", because that is the composing lesson and it is a real constraint on what a student may make. The teacher-facing obligation is recorded where it belongs, in the planning folder's `CLAUDE.md` and the Term 1 resources README, not repeated on student pages. ICIP is not assessed content in this course: NESA's "Safe and ethical practices" content group, where ICIP protocols sit, belongs to Music Life Skills and is stated to have no Music 1 equivalent. The ink border is what keeps it visually distinct from an actual Term 2 on-air fill, which is the same yellow with no border.
